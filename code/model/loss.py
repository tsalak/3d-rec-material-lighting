...
class IDRLoss(nn.Module):
    def __init__(self, idr_rgb_weight, our_rgb_weight, eikonal_weight, mask_weight, alpha,
                 r_patch=-1, normalsmooth_weight=0., loss_type='L1'):
        super().__init__()
        self.idr_rgb_weight = idr_rgb_weight
        self.our_rgb_weight = our_rgb_weight
    ...
    def get_rgb_loss(self, idr_rgb_values, our_rgb_values, rgb_gt, network_object_mask, object_mask):
        ...
        our_rgb_values = our_rgb_values[mask].reshape((-1, 3))
        rgb_gt = rgb_gt.reshape(-1, 3)[mask].reshape((-1, 3))

        idr_rgb_loss = self.img_loss(idr_rgb_values, rgb_gt)
        our_rgb_loss = self.img_loss(our_rgb_values, rgb_gt)

        return idr_rgb_loss, our_rgb_loss
    ...
    def forward(self, model_outputs, ground_truth):
        ...
        idr_rgb_loss, our_rgb_loss = self.get_rgb_loss(model_outputs['idr_rgb_values'], model_outputs['our_rgb_values'],
        ...
        loss = self.idr_rgb_weight * idr_rgb_loss + \
               self.our_rgb_weight * our_rgb_loss + \
