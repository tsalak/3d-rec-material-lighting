...
class DTUSceneDataset(torch.utils.data.Dataset):
    """Dataset for a class of objects, where each datapoint is a SceneInstanceDataset."""
    ...
    def __init__(self,
        ...
        world_mats = list()
        idx = 0
        while len(scale_mats) < self.n_images:
            if f'{image_dir}/{idx:06d}.png' in image_paths:
                scale_mats.append(camera_dict['scale_mat_%d' % idx].astype(np.float32))
                world_mats.append(camera_dict['world_mat_%d' % idx].astype(np.float32))
            idx += 1
