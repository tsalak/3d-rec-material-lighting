...
class OurTrainRunner():
    ...
    def __init__(self,**kwargs):
        ...
        self.light_func = kwargs['light_func']
        self.dataset_type = kwargs['dataset_type']

        self.expname = self.conf.get_string('train.expname') + '-' + kwargs['expname']
        
        if self.light_func == 0:
            self.light_func_dir = "SphericalGaussians"
        elif self.light_func == 1:
            self.light_func_dir = "SphericalHarmonics"
        elif self.light_func == 2:
            self.light_func_dir = "SphericalWavelets"
        
        if kwargs['is_continue'] and kwargs['timestamp'] == 'latest':
            if os.path.exists(os.path.join('../', kwargs['exps_folder_name'], self.expname, self.light_func_dir)):
                timestamps = os.listdir(os.path.join('../', kwargs['exps_folder_name'], self.expname, self.light_func_dir))
                if (len(timestamps)) == 0:
                    is_continue = False
                    timestamp = None
                else:
        ...
        utils.mkdir_ifnotexists(os.path.join(self.expdir, self.light_func_dir, self.timestamp))

        self.plots_dir = os.path.join(self.expdir, self.light_func_dir, self.timestamp, 'plots')
        utils.mkdir_ifnotexists(self.plots_dir)

        # create checkpoints dirs
        self.checkpoints_path = os.path.join(self.expdir, self.light_func_dir, self.timestamp, 'checkpoints')
        ...
        self.our_optimizer_params_subdir = "OurOptimizerParameters"
        self.our_scheduler_params_subdir = "OurSchedulerParameters"
        ...
        utils.mkdir_ifnotexists(os.path.join(self.checkpoints_path, self.our_optimizer_params_subdir))
        utils.mkdir_ifnotexists(os.path.join(self.checkpoints_path, self.our_scheduler_params_subdir))

        print("Write tensorboard to: ", os.path.join(self.expdir, self.light_func_dir, self.timestamp))
        self.writer = SummaryWriter(os.path.join(self.expdir, self.light_func_dir, self.timestamp))
        ...
        if self.dataset_type == 0:
            # synthetic dataset
            self.train_dataset = utils.get_class(self.conf.get_string('dataset.synthetic.class'))(kwargs['gamma'],
            ...
            self.plot_dataset = utils.get_class(self.conf.get_string('dataset.synthetic.class'))(kwargs['gamma'],
                                                kwargs['data_split_dir'], self.train_cameras)
            self.plot_dataloader = DataLoader(self.plot_dataset,
            ...
            # DTU dataset
            dataset_conf = self.conf.get_config('dataset.DTU.conf')
            if kwargs['scan_id'] != -1:
                dataset_conf['scan_id'] = kwargs['scan_id']
            dataset_conf['gamma'] = kwargs['gamma']

            self.train_dataset = utils.get_class(self.conf.get_string('dataset.DTU.class'))(self.train_cameras,
                                                                                          **dataset_conf)
        ...
        self.model = utils.get_class(self.conf.get_string('train.model_class'))(conf=self.conf.get_config('model'), light_func=self.light_func)
        ...
        self.our_optimizer = torch.optim.Adam(self.model.envmap_material_network.parameters(),
                                              lr=self.conf.get_float('train.our_learning_rate'))
        
        self.our_scheduler = torch.optim.lr_scheduler.MultiStepLR(self.our_optimizer,
                                                                self.conf.get_list('train.our_sched_milestones', default=[]),
                                                                gamma=self.conf.get_float('train.our_sched_factor', default=0.0))
        ...
        self.start_epoch = 0
        if is_continue:
            old_checkpnts_dir = os.path.join(self.expdir, self.light_func_dir, timestamp, 'checkpoints')
            ...
            data = torch.load(
                os.path.join(old_checkpnts_dir, self.our_optimizer_params_subdir, str(kwargs['checkpoint']) + ".pth"))
            self.our_optimizer.load_state_dict(data["optimizer_state_dict"])

            data = torch.load(
                os.path.join(old_checkpnts_dir, self.our_scheduler_params_subdir, str(kwargs['checkpoint']) + ".pth"))
            self.our_scheduler.load_state_dict(data["scheduler_state_dict"])
    ...
    def save_checkpoints(self, epoch):
        ...
        torch.save(
            {"epoch": epoch, "optimizer_state_dict": self.our_optimizer.state_dict()},
            os.path.join(self.checkpoints_path, self.our_optimizer_params_subdir, str(epoch) + ".pth"))
        torch.save(
            {"epoch": epoch, "optimizer_state_dict": self.our_optimizer.state_dict()},
            os.path.join(self.checkpoints_path, self.our_optimizer_params_subdir, "latest.pth"))
        
        torch.save(
            {"epoch": epoch, "scheduler_state_dict": self.our_scheduler.state_dict()},
            os.path.join(self.checkpoints_path, self.our_scheduler_params_subdir, str(epoch) + ".pth"))
        torch.save(
            {"epoch": epoch, "scheduler_state_dict": self.our_scheduler.state_dict()},
            os.path.join(self.checkpoints_path, self.our_scheduler_params_subdir, "latest.pth"))
    ...
    def plot_to_disk(self):
        ...
        for s in split:
            ...
            res.append({
                ...
                'our_rgb_values': out['our_rgb_values'].detach(),
                'our_diffuse_albedo_values': out['our_diffuse_albedo_values'].detach(),
        ...
        # log environment map
        envmap = self.model.compute_envmap(H=256, W=512)
        envmap = envmap.cpu().detach().numpy()
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.nepochs + 1):
            ...
            for data_index, (indices, model_input, ground_truth) in enumerate(self.train_dataloader):
                ...
                self.our_optimizer.zero_grad()
                if self.train_cameras:
                    self.optimizer_cam.zero_grad()
                
                loss.backward()

                if self.light_func == 2:
                    if self.model.envmap_material_network.train_bandwidth:
                        self.model.envmap_material_network.backward()

                self.idr_optimizer.step()
                self.our_optimizer.step()
                if self.train_cameras:
                    self.optimizer_cam.step()

                if self.cur_iter % 50 == 0:
                    roughness, shininess, specular_albedo, bandwidth_parameter = self.model.envmap_material_network.get_base_materials()
                    print(
                        '{} [{}/{}] ({}/{}): loss = {}, idr_rgb_loss = {}, our_rgb_loss = {}, eikonal_loss = {}, '
                        'mask_loss = {}, normalsmooth_loss = {}, alpha = {}, idr_lr = {}, our_lr = {}, idr_psnr = {}, our_psnr = {}, '
                        'roughness = {}, shininess = {}, bandwidth_parameter = {}, specular_albedo = {}, idr_rgb_weight = {}, our_rgb_weight = {}, mask_weight = {}, eikonal_weight = {}, '
                        'normal_smooth_weight = {} '
                            .format(self.expname, epoch, self.cur_iter, data_index, self.n_batches, loss.item(),
                                    ...
                                    loss_output['our_rgb_loss'].item(),
                                    loss_output['eikonal_loss'].item(),
                                    loss_output['mask_loss'].item(),
                                    loss_output['normalsmooth_loss'].item(),
                                    self.loss.alpha,
                                    self.idr_scheduler.get_lr()[0],
                                    self.our_scheduler.get_lr()[0],
                                    mse2psnr(loss_output['idr_rgb_loss'].item()),
                                    mse2psnr(loss_output['our_rgb_loss'].item()),
                                    roughness[0, 0].item(), shininess[0, 0].item(), bandwidth_parameter[0, 0].item(), specular_albedo[0, 0].item(),
                                    self.loss.idr_rgb_weight, self.loss.our_rgb_weight, self.loss.mask_weight,
                    ...
                    self.writer.add_scalar('our_rgb_loss', loss_output['our_rgb_loss'].item(), self.cur_iter)
                    self.writer.add_scalar('our_psnr', mse2psnr(loss_output['our_rgb_loss'].item()), self.cur_iter)
                    ...
                    self.writer.add_scalar('our_rgb_weight', self.loss.our_rgb_weight, self.cur_iter)
                    ...
                    if shininess is not None:
                        self.writer.add_scalar('shininess', shininess[0, 0].item(), self.cur_iter)
                    ...
                    self.writer.add_scalar('our_lrate', self.our_scheduler.get_lr()[0], self.cur_iter)

                self.cur_iter += 1

                self.idr_scheduler.step()
                self.our_scheduler.step()
